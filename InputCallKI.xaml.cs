...
namespace Pricer_v5
{
    ...
    /// </summary>
    public partial class InputCallKI : Window
    {
        public InputCallKI()
        {
            InitializeComponent();
        ...
        private async void DisplayData()
        {
                ...
                await Task.Run(() =>
                {
                    for (int i = 0; i < 100000; i++)
                    {
                        ...
                        else
                        {
                            ...
                            if (cal.BarriereReachCallKO(L, traj) == false)
                            {
                                Prime = Prime + 0.0m;
                            }
                            else
            ...
            // PLOT PAYOFF
            var point2 = cal.PlotPayoff(S, K, Prime / 100000m, "CallKI", L);
            ...
            decimal PrimeFa;
            PrimeFa = Math.Round(cal.CallKI(S, r, v, Tfa, K, L), 5);
            FAInp.Text = PrimeFa.ToString();
            MCInp.Text = Prime.ToString();
            DeltaInp.Text = Math.Round(cal.DeltaKIFDM(S, r, v, Tfa, K, L, "Call"), 5).ToString();
            GammaInp.Text = Math.Round(cal.GammaKIFDM(S, r, v, Tfa, K, L, "Call"), 5).ToString();
            VegaInp.Text = Math.Round(cal.VegaKIFDM(S, r, v, Tfa, K, L, "Call"), 5).ToString();
            ThetaInp.Text = Math.Round(cal.ThetaKIFDM(S, r, v, Tfa, K, L, "Call"), 5).ToString();
            RhoInp.Text = Math.Round(cal.RhoKIFDM(S, r, v, Tfa, K, L, "Call"), 5).ToString();
        }
