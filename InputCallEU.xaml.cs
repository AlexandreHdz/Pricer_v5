...
namespace Pricer_v5
{
    ...
    public partial class InputCallEU : Window
    {
        ...
        private async void DisplayData()
        {
            ...
            double Prix = cal.call_price(S, K, r, v, T / (double)nbJoursTot);
            ...
            Prime = (decimal)Math.Exp(-r * T / (double)nbJoursTot) * Prime / 100000m;
            Prime = Math.Round(Prime, 5);
            MCInp.Text = Prime.ToString();
            DeltaInp.Text = Math.Round(cal.call_delta(S, r, v, T / (double)nbJoursTot, K), 5).ToString();
            GammaInp.Text = Math.Round(cal.call_gamma(S, r, v, T / (double)nbJoursTot, K), 5).ToString();
            VegaInp.Text = Math.Round(cal.call_vega(S, r, v, T / (double)nbJoursTot, K), 5).ToString();
            ThetaInp.Text = Math.Round(cal.call_theta(S, r, v, T / (double)nbJoursTot, K), 5).ToString();
            RhoInp.Text = Math.Round(cal.call_rho(S, r, v, T / (double)nbJoursTot, K), 5).ToString();
        }
